...
- Store embeddings for chunks only when allowed by tenant/provider policy.
- Keep provider abstraction.
- Do not put raw confidential text in analytics.
- Support deletion of embeddings when document/case deleted.
- Track model/provider/version per AgentRun.
- Evaluation sets must use approved synthetic or consented documents.
