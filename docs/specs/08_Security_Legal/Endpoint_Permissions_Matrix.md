# Advokat AI — Endpoint Permissions Matrix
...
- Tenant and case isolation enforced server-side.
- Least privilege by default.
- MFA required for pilot users where auth provider supports it.
- Encryption in transit and at rest.
- Signed URLs expire quickly.
- No document content in analytics.
- No secrets in logs.
- Audit events append-only.
- AI provider calls must be logged as `AgentRun` with provider metadata.
- No model training on customer data unless explicitly contracted.
- Redaction/privacy review before external sharing/export.
- Admin actions audited.

## Compliance gates

Before Pilot Candidate:
- data storage region chosen
...
- DPIA
- AI Act/legal classification review
- penetration test
- vendor risk review
- security review
