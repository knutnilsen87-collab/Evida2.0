...
Advokat AI is a production-grade legal workspace for document-driven case work. It turns uploaded legal documents into structured source-backed facts, evidence, chronology, arguments, risks and draft documents, while enforcing control gates before risky actions.

The product succeeds when a legal professional can upload a real case file, understand the document basis, build a reliable factual picture, generate source-controlled drafts, and export only after the system verifies source coverage, unresolved risks, privacy constraints and required approvals.

The product's UX is built around a calm AI legal assistant that acts as a collaboration partner and program guide. The assistant must reduce mental load and prevent user falloff by explaining the program, recommending usage, walking the user through actions and making complex legal workflows feel simple.

## Strategic bet

Legal users do not need more raw AI output. They need controlled, explainable, source-grounded legal work assistance they can trust under professional responsibility.

## Release strategy

Do not build a toy MVP. Build a Pilot Candidate that can handle real legal documents under controlled conditions, then harden into Official Release.
