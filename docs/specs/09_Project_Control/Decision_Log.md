...
|---|---|---|---|
| ADR-001 | No MVP; build Pilot Candidate then Official Release | Accepted | Real legal documents require production-near controls |
| ADR-002 | Source-backed claims required | Accepted | Prevent unsupported legal output |
| ADR-003 | AI assistant as primary learning/onboarding surface | Accepted | Reduces falloff and mental load |
| ADR-004 | Monorepo | Accepted | Shared contracts and faster coordinated development |
| ADR-005 | Canonical domain objects only | Accepted | Prevents schema drift |
| ADR-006 | Export requires control gate and human approval | Accepted | Legal/safety requirement |
| ADR-007 | Provider abstraction for LLM/OCR | Accepted | Avoid vendor lock-in and support privacy options |
