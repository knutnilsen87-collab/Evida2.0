...
Premium, calm, modern, precise, trustworthy. Avoid heavy admin-panel feel and flashy AI showcase.

## Components

- App shell: top bar, left navigation, main work area, right assistant/context panel, bottom status strip.
- Object card: title, status, source coverage, risk chip, next action.
- Control gate modal: clear reason, affected objects, recommended action, approval button.
- Assistant panel: conversational, context-aware, with guided-action buttons.
- Document viewer: page view, OCR status, highlights, source anchors.
- Draft editor: source coverage per paragraph.
- Risk table: severity, object, reason, mitigation, status.

## Status colors

- Neutral: normal information
- Blue/indigo: active context / primary action
- Yellow/amber: warning / needs attention
- Red: blocked / critical
- Green: verified / approved
- Purple: assistant/AI suggestion
