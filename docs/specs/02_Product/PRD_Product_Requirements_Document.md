# Advokat AI — PRD Product Requirements Document
...
### P0 requirements for Pilot Candidate

1. User can create a case.
2. User can upload legal documents.
3. System processes documents into pages, OCR text, chunks and hashes.
4. User can inspect document processing status.
5. System extracts candidate facts with source references.
6. User can approve, reject or edit fact claims.
7. User can build chronology from fact claims.
8. User can create evidence map.
9. User can create a draft document with paragraph-level source coverage.
10. Export gate blocks unsafe export.
11. AI assistant can explain every screen, warning and next step.
12. Audit trail records all critical actions.
13. Role-based access controls case data.
14. Admin can review tenant/case/user state.

### P1 requirements for Release Candidate

- Legal source verification workflow.
- Full audit export.
- Advanced risk/motargument handling.
- Performance hardening for large case files.
- Security review and penetration test fixes.
- Compliance documentation and DPIA evidence.
