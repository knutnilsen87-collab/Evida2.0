...
### Source rules
- A `FactClaim` may exist without source only if `claim_status = missing_source`.
- A `FactClaim` with `missing_source` cannot be used in export-ready draft without explicit unresolved-risk marker.
- Draft paragraphs containing factual claims require at least one `SourceRef`.
- OCR uncertainty above threshold creates `RiskItem`.

### Export rules
- Export requires passed Export Gate.
- Export requires approval from role `case_owner`, `legal_reviewer` or `admin`.
- Export package must include export hash and audit reference.
- Export must include source appendix when configured.

### Access rules
- Users see only tenants and cases they belong to.
- External collaborators get least privilege by default.
- Client/external portal access is disabled until explicitly enabled.

### AI rules
- AI suggestions are suggestions, not legal truth.
- AI must disclose uncertainty.
- AI may not delete source documents.
- AI may not approve its own output.
- AI must produce `AgentRun` record for material operations.
