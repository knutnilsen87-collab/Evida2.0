# Advokat AI — Functional Specification
...
### Case room
- Create, view, update, archive cases.
- Isolated memory, documents, agents, audit and drafts per case.
- Case dashboard shows document status, source coverage, risks, drafts, next action.

### Document ingestion
- Upload PDF/images/emails where supported.
- Store immutable original.
- Calculate SHA-256.
- Extract pages.
- OCR pages.
- Generate chunks.
- Assign Bates/Exhibit labels.
- Create processing status and error states.

### Source graph
- Link `FactClaim` to `SourceRef`.
- Link `TimelineEvent`, `EvidenceItem`, `Argument`, `DraftDocument` paragraphs to source-backed facts.
- Mark unsupported or uncertain claims.

### Drafting
- User selects draft type.
- Assistant proposes structure.
- Draft paragraphs carry source coverage.
- Missing source paragraphs are visually marked.
- Export blocked until control gates pass.

### AI assistant
- Available globally.
- Context-aware.
- Can explain program features and legal workflow.
- Can guide user step by step.
- Cannot bypass gates.
