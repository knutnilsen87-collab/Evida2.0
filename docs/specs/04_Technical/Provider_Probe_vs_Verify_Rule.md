# Advokat AI — Provider Probe vs Verify Rule
...
### Document processing
`queued -> virus_scan -> stored -> page_extracting -> ocr_running -> chunking -> indexing -> source_anchor_created -> completed`

Failure states:
`failed_virus_scan | failed_extract | failed_ocr | failed_chunking | failed_indexing | quarantine`

### Agent run
`queued -> context_loading -> planning -> gated -> running -> output_validating -> completed`

Blocked states:
`policy_blocked | insufficient_context | approval_required | provider_failed | validation_failed`

## Probe vs verify

- Probe: cheap check that informs next action, not enough for release/export.
- Verify: authoritative check that can pass a gate.

Example:
- OCR confidence estimate = probe.
- Page text with hash, source anchors and user-visible status = verify.
